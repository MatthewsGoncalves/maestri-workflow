...
append_project_overlay() {
  if [[ -f "$ROOT/rules/project/git.md" ]]; then
    echo ""
    echo "## Regras do projeto (overlay)"
    cat "$ROOT/rules/project/git.md"
    echo ""
    [[ -f "$ROOT/rules/project/communications.md" ]] && cat "$ROOT/rules/project/communications.md" && echo ""
    [[ -f "$ROOT/rules/project/paths.md" ]] && cat "$ROOT/rules/project/paths.md"
  fi
}
...
create_role() {
  ...
  local skills_file="${4:-}"
  local prompt
  prompt="$(compile_prompt "$rules_file" "$ops_file")"
